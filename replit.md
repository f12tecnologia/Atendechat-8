...
## Recent Changes
- **Dec 28, 2025**: Fixed duplicate contacts and media display - (1) Removed UNIQUE constraint from Contact.number and added composite constraint (number + companyId) via migration to allow same number across different companies. (2) Fixed processMediaMessage to return `/public/${fileName}` URL directly instead of just filename, enabling frontend to display images, videos, documents immediately without additional URL construction.
